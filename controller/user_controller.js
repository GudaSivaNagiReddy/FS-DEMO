...
exports.register = (req, res, next) => {
	...
	res.json({ msg: 'Password must be at least 3 characters' });
	}
	else {
	// Checking if user exists
	User.findOne({ email: email }).then(user => {
		if (user) {
		res.json({msg:'This email is already exist.Please Login using this email'});
		}
		// Creating the user 
		else {
		...
		bcrypt.genSalt(10, (err, salt) => {
			...
			newUser
				...
				res.json({msg:"Successfully Register.You can login"});
				})
